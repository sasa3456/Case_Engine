...
namespace Case_Engine
{
	...
	class Renderer
	{
		static constexpr uint32_t AO_NOISE_DIM = 8;
		static constexpr uint32_t SSAO_KERNEL_SIZE = 16;
		static constexpr uint32_t RESOLUTION = 512;
		static constexpr uint32_t VOXEL_RESOLUTION = 128;
		static constexpr uint32_t VOXELIZE_MAX_LIGHTS = 8;
		static constexpr uint32_t CLUSTER_SIZE_X = 16;
		static constexpr uint32_t CLUSTER_SIZE_Y = 16;
		static constexpr uint32_t CLUSTER_SIZE_Z = 16;
	...
	public:
		...
		Renderer(tecs::registry& reg, GfxDevice* gfx, uint32_t width, uint32_t height); 
		~Renderer();
	...
	private:
		...
		void CreateBokehViews(uint32_t width, uint32_t height);
		void CreateRenderTargets(uint32_t width, uint32_t height);
		void CreateGBuffer(uint32_t width, uint32_t height);
		void CreateAOTexture(uint32_t width, uint32_t height);
		void CreateRenderPasses(uint32_t width, uint32_t height);
		void CreateComputeTextures(uint32_t width, uint32_t height);
