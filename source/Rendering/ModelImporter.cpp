...
namespace Case_Engine
{
		...
		void GenerateTerrainLayerTexture(char const* texture_name, Terrain* terrain, TerrainTextureLayerParameters const& params)
		{
			...
			for (uint64_t j = 0; j < depth; ++j)
			{
				for (uint64_t i = 0; i < width; ++i)
				{
					...
					uint32_t sum = temp_layer_data[(j * width + i) * 4 + 0]
			...
			for (size_t j = 2; j < depth - 2; ++j)
			{
				for (size_t i = 2; i < width - 2; ++i)
				{
					...
					{
						for (int32_t l = -2; l <= 2; ++l)
						{
							n1 += (int32_t)temp_layer_data[((j + k) * width + i + l) * 4 + 0];
							n2 += (int32_t)temp_layer_data[((j + k) * width + i + l) * 4 + 1];
							n3 += (int32_t)temp_layer_data[((j + k) * width + i + l) * 4 + 2];
						}
					}
			...
			WriteImageTGA(texture_name, layer_data, (int32_t)width, (int32_t)depth);
		}
    }
    ...
    std::vector<entity> ModelImporter::LoadGrid(GridParameters const& params, std::vector<TexturedNormalVertex>* vertices_out)
    {
        if (params.heightmap)
        {
			CASE_ENGINE_ASSERT(params.heightmap->Depth() == params.tile_count_z + 1);
			CASE_ENGINE_ASSERT(params.heightmap->Width() == params.tile_count_x + 1);
        }

        std::vector<entity> chunks;
        std::vector<TexturedNormalVertex> vertices{};
        for (uint64_t j = 0; j <= params.tile_count_z; j++)
        {
            for (uint64_t i = 0; i <= params.tile_count_x; i++)
            {
        ...
        if (!params.split_to_chunks)
        {
            ...
            uint32_t i1 = 0;
            uint32_t i2 = 1;
            uint32_t i3 = static_cast<uint32_t>(i1 + params.tile_count_x + 1);
            uint32_t i4 = static_cast<uint32_t>(i2 + params.tile_count_x + 1);
            for (uint64_t i = 0; i < params.tile_count_x * params.tile_count_z; ++i)
            {
        ...
        else
        {
            ...
            for (size_t j = 0; j < params.tile_count_z; j += params.chunk_count_z)
            {
                for (size_t i = 0; i < params.tile_count_x; i += params.chunk_count_x)
                {
                    ...
                    uint32_t const indices_count = static_cast<uint32_t>(params.chunk_count_z * params.chunk_count_x * 3 * 2);
                    uint32_t const indices_offset = static_cast<uint32_t>(indices.size());
                    ...
                    for (size_t k = j; k < j + params.chunk_count_z; ++k)
                    {
                        ...
                        {
                            uint32_t i1 = static_cast<uint32_t>(k * (params.tile_count_x + 1) + m);
                            uint32_t i2 = static_cast<uint32_t>(i1 + 1);
                            uint32_t i3 = static_cast<uint32_t>((k + 1) * (params.tile_count_x + 1) + m);
                            uint32_t i4 = static_cast<uint32_t>(i3 + 1);
	...
	std::vector<entity> ModelImporter::LoadObjMesh(std::string const& model_path, std::vector<std::string>* diffuse_textures_out)
	{
		...
		if (!reader.ParseFromFile(model_path, reader_config))
		{
            ...
            {
				CASE_ENGINE_LOG(ERROR, reader.Error().c_str());
            }
			return {};
		}
		if (!reader.Warning().empty())
		{
			CASE_ENGINE_LOG(WARNING, reader.Warning().c_str());
		}
		...
		for (size_t s = 0; s < shapes.size(); s++)
		{
			...
			mesh_component.vertex_count = static_cast<uint32_t>(vertices.size());
			...
			{
				CASE_ENGINE_ASSERT(shapes[s].mesh.material_ids.size() > 0);
				int material_id = shapes[s].mesh.material_ids[0];
				CASE_ENGINE_ASSERT(material_id >= 0);
				tinyobj::material_t material = materials[material_id];
				diffuse_textures_out->push_back(material.diffuse_texname);
			}

		}
		CASE_ENGINE_LOG(INFO, "OBJ Mesh %s successfully loaded!", model_path.c_str());
		return entities;
	}
	...
	std::vector<entity> ModelImporter::ImportModel_GLTF(ModelParameters const& params)
	{
		...
		if (!warn.empty())
		{
			CASE_ENGINE_LOG(WARNING, warn.c_str());
		}
		if (!err.empty())
		{
			CASE_ENGINE_LOG(ERROR, err.c_str());
			return {};
		}
		if (!ret)
		{
			CASE_ENGINE_LOG(ERROR, "Failed to load model %s", model_name.c_str());
			return {};
		}
		...
		for (auto& mesh : model.meshes)
		{
			...
			{
				CASE_ENGINE_ASSERT(primitive.indices >= 0);
				...
				Mesh mesh_component{};
				mesh_component.indices_count = static_cast<uint32_t>(index_accessor.count);
				mesh_component.start_index_location = static_cast<uint32_t>(indices.size());
				mesh_component.base_vertex_location = static_cast<uint32_t>(vertices.size());
				...
				uint32_t vertex_offset = mesh_component.base_vertex_location;
				uint32_t index_count = mesh_component.indices_count;
				uint32_t index_offset = mesh_component.start_index_location;
				...
				for (auto& attr : primitive.attributes)
				{
					...
					else if (!attr_name.compare("TEXCOORD_0"))
					{
						...
						else if (accessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE)
						{
							for (size_t i = 0; i < vertex_count; ++i)
							{
								uint8_t const& s = *(uint8_t*)((size_t)data + i * stride + 0 * sizeof(uint8_t));
								uint8_t const& t = *(uint8_t*)((size_t)data + i * stride + 1 * sizeof(uint8_t));
						...
						else if (accessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT)
						{
							for (size_t i = 0; i < vertex_count; ++i)
							{
								uint16_t const& s = *(uint16_t*)((size_t)data + i * stride + 0 * sizeof(uint16_t));
								uint16_t const& t = *(uint16_t*)((size_t)data + i * stride + 1 * sizeof(uint16_t));
				...
				CASE_ENGINE_ASSERT(tangent_handness.size() == tangents.size());
				uint64_t vertex_count = positions.size();
				...
				mesh_component.vertex_count = (uint32_t)vertex_count;
				reg.emplace<Mesh>(e, mesh_component);
				if (has_tangents)
				{
		...
		relationship.children_count = (uint32_t)entities.size();
		CASE_ENGINE_ASSERT(relationship.children_count <= Relationship::MAX_CHILDREN);
		...
		CASE_ENGINE_LOG(INFO, "GLTF Mesh %s successfully loaded!", params.model_path.c_str());
		return entities;
	}
    ...
    entity ModelImporter::LoadLight(LightParameters const& params)
    {
        ...
        {
            uint32_t const size = params.mesh_size;
			...
			else if (params.light_data.type == LightType::Point)
				material.albedo_texture = g_TextureManager.LoadTexture(paths::TexturesDir() + "light_bulb.png");
            ...
            { 
				CASE_ENGINE_LOG(ERROR, "Light with quad mesh needs diffuse texture!");
            }

			if (params.light_data.type == LightType::Point)
				material.shader = ShaderProgram::Volumetric_Point;
			else if (material.albedo_texture != INVALID_TEXTURE_HANDLE)
				material.shader = ShaderProgram::Billboard;
			else
			{
			...
			if (params.light_data.type != LightType::Point) reg.emplace<Forward>(light, true);
        }
	...
	entity ModelImporter::LoadFoliage(FoliageParameters const& params)
	{
		...
		CASE_ENGINE_ASSERT(foliages.size() == 1);
		entity foliage = foliages[0];

		std::vector<FoliageInstance> instance_data{};
		for (int32_t i = 0; i < params.foliage_count; ++i)
		{
            static const uint32_t MAX_ITERATIONS = 5;
			Vector3 position{};
			Vector3 normal{};
		...
		mesh_component.instance_count = (uint32_t)instance_data.size();
    ...
    std::vector<entity> ModelImporter::LoadTrees(TreeParameters const& params)
	{
		...
		CASE_ENGINE_ASSERT(diffuse_textures.size() == trees.size());

		std::vector<TreeInstance> instance_data{};
		for (int32_t i = 0; i < params.tree_count; ++i)
		{
			static const uint32_t MAX_ITERATIONS = 5;
			Vector3 position{};
			Vector3 normal{};
