...
namespace Case_Engine
{
	...
	struct COMPONENT Mesh
	{
		...
		uint32_t start_vertex_location = 0; //Index of the first vertex

		//vb/ib
		uint32_t indices_count = 0;
		uint32_t start_index_location = 0; //The location of the first index read by the GPU from the index buffer
		int32_t base_vertex_location = 0;  //A value added to each index before reading a vertex from the vertex buffer

		//instancing
		uint32_t instance_count = 1;
		uint32_t start_instance_location = 0; //A value added to each index before reading per-instance data from a vertex buffer
	...
	struct COMPONENT Emitter
	{
		...
		int32_t					collision_thickness = 40;
