...
namespace Case_Engine
{
	ParticleRenderer::ParticleRenderer(GfxDevice* gfx) : gfx{ gfx },
		dead_list_buffer(gfx, AppendBufferDesc(MAX_PARTICLES, sizeof(uint32_t))),
		...
		indirect_render_args_buffer(gfx, IndirectArgsBufferDesc(5 * sizeof(uint32_t))),
		indirect_sort_args_buffer(gfx, IndirectArgsBufferDesc(4 * sizeof(uint32_t))),
	...
	void ParticleRenderer::Update(float dt, Emitter& emitter_params)
	{
		...
		if (emitter_params.particles_per_second > 0.0f)
		{
			...
			if (emitter_params.accumulation > 1.0f)
			{
				...
				emitter_params.number_to_emit = (int32_t)integer_part;
	...
	void ParticleRenderer::InitializeDeadList() 
	{
		...
		command_context->Dispatch((uint32_t)std::ceil(MAX_PARTICLES * 1.0f / 256), 1, 1);
	...
	void ParticleRenderer::ResetParticles()
	{
		...
		uint32_t initial_counts[] = { (uint32_t)-1, (uint32_t)-1 };
	...
	{
		GfxCommandContext* command_context = gfx->GetCommandContext();
		CaseEngineGfxScopedAnnotation(command_context,"Particles Emit Pass");
		...
		if (emitter_params.number_to_emit > 0)
		{
			...
			uint32_t initial_counts[] = { (uint32_t)-1, (uint32_t)-1, (uint32_t)-1 };
			command_context->SetShaderResourcesRW(0, uavs, initial_counts);
			...
			uint32_t thread_groups_x = (uint32_t)std::ceil(emitter_params.number_to_emit * 1.0f / 1024);
	...
	{
		GfxCommandContext* command_context = gfx->GetCommandContext();
		CaseEngineGfxScopedAnnotation(command_context, "Particles Simulate Pass");
		...
		uint32_t initial_counts[] = { (uint32_t)-1, (uint32_t)-1, (uint32_t)-1, 0, (uint32_t)-1, (uint32_t)-1 };
		command_context->SetShaderResourcesRW(0, uavs, initial_counts);
	...
	{
		GfxCommandContext* command_context = gfx->GetCommandContext();
		CaseEngineGfxScopedAnnotation(command_context, "Particles Rasterize Pass");
	...
	{
		GfxCommandContext* command_context = gfx->GetCommandContext();
		CaseEngineGfxScopedAnnotation(command_context, "Particles Sort Pass");
	...
	bool ParticleRenderer::SortInitial()
	{
		GfxCommandContext* command_context = gfx->GetCommandContext();
		bool done = true;
		uint32_t numThreadGroups = ((MAX_PARTICLES - 1) >> 9) + 1;
	...
	bool ParticleRenderer::SortIncremental(uint32_t presorted)
	{
		GfxCommandContext* command_context = gfx->GetCommandContext();
		...
		uint32_t merge_size = presorted * 2;
		for (uint32_t merge_subsize = merge_size >> 1; merge_subsize > 256; merge_subsize = merge_subsize >> 1)
		{
