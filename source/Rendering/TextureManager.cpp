...
TextureHandle TextureManager::LoadTexture(std::wstring const& name)
{
	...
	default:
		CASE_ENGINE_ASSERT(false && "Unsupported Texture Format!");
	}
...
{
	TextureFormat format = GetTextureFormat(name);
	CASE_ENGINE_ASSERT(format == TextureFormat::DDS || format == TextureFormat::HDR && "Cubemap in one file has to be .dds or .hdr format");
...
TextureHandle TextureManager::LoadCubeMap(std::array<std::string, 6> const& cubemap_textures)
{
	...
	CASE_ENGINE_ASSERT(format == TextureFormat::JPG || format == TextureFormat::PNG || format == TextureFormat::TGA ||
