...
void FFT_VerticalCS(uint3 groupId : SV_GroupID, uint3 groupThreadId : SV_GroupThreadID)
{
    uint2 pixelCoord = uint2(groupId.x, groupThreadId.x);
    ...
    OutputTx[int2(pixelCoord.x, outIdx + SubseqCount)] = float4(result0.zw, result1.zw);
}