	...
	namespace GfxShaderCompiler
	{
			...
			GfxShaderCompileOutput& output)
		{
			...
			case GfxShaderStage::CS:
				...
				break;
			default:
				CASE_ENGINE_ASSERT_MSG(false, "Unsupported Shader Stage!");
			}
			...
			uint64_t macro_hash = crc64(macro_key.c_str(), macro_key.size());
			...
			CASE_ENGINE_LOG(INFO, "Shader '%s.%s' not found in cache. Compiling...", input.source_file.c_str(), default_entrypoint.c_str());
		...
		compile:
			...
			if (FAILED(hr))
			{
				if (error_blob)
				{
					...
					CASE_ENGINE_LOG(ERROR, "%s", err_msg);
					std::string msg = "Click OK after you have fixed the following errors: \n";
					msg += err_msg;
					int32_t result = MessageBoxA(NULL, msg.c_str(), NULL, MB_OKCANCEL);
			...
			uint64_t shader_hash = crc64((char*)bytecode_blob->GetBufferPointer(), bytecode_blob->GetBufferSize());
