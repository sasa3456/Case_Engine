...
namespace Case_Engine
{
	...
	struct GfxTextureSubresourceDesc
	{
		...
		uint32_t slice_count = static_cast<uint32_t>(-1);
		uint32_t first_mip = 0;
		uint32_t mip_count = static_cast<uint32_t>(-1);
	...
	class GfxTexture
	{
		...
		{
			CASE_ENGINE_ASSERT(desc.type == TextureType_1D);
		...
		{
			CASE_ENGINE_ASSERT(desc.type == TextureType_2D);
		...
		{
			CASE_ENGINE_ASSERT(desc.type == TextureType_3D);
	...
	public:
			...
			: gfx(gfx), desc(desc)
		{
			...
			{
				const_cast<GfxTextureDesc&>(desc).mip_levels = (uint32_t)log2(std::max(desc.width, desc.height)) + 1;
			}
		...
		[[maybe_unused]] uint64_t CreateSRV(GfxTextureSubresourceDesc const* desc = nullptr)
		{
			GfxTextureSubresourceDesc _desc = desc ? *desc : GfxTextureSubresourceDesc{};
			return CreateDescriptor(GfxSubresourceType_SRV, _desc);
		}
		[[maybe_unused]] uint64_t CreateUAV(GfxTextureSubresourceDesc const* desc = nullptr)
		{
			GfxTextureSubresourceDesc _desc = desc ? *desc : GfxTextureSubresourceDesc{};
			return CreateDescriptor(GfxSubresourceType_UAV, _desc);
		}
		[[maybe_unused]] uint64_t CreateRTV(GfxTextureSubresourceDesc const* desc = nullptr)
		{
			GfxTextureSubresourceDesc _desc = desc ? *desc : GfxTextureSubresourceDesc{};
			return CreateDescriptor(GfxSubresourceType_RTV, _desc);
		}
		[[maybe_unused]] uint64_t CreateDSV(GfxTextureSubresourceDesc const* desc = nullptr)
		{
			GfxTextureSubresourceDesc _desc = desc ? *desc : GfxTextureSubresourceDesc{};
		...
		GfxRenderTarget		RTV(uint64_t i = 0) const { return rtvs[i].Get(); }
		GfxDepthTarget		DSV(uint64_t i = 0) const { return dsvs[i].Get(); }
	...
	private:
		[[maybe_unused]] uint64_t CreateDescriptor(GfxSubresourceType type, GfxTextureSubresourceDesc const& view_desc)
		{
			uint32_t first_slice = view_desc.first_slice;
			uint32_t slice_count = view_desc.slice_count;
			uint32_t first_mip = view_desc.first_mip;
			uint32_t mip_count = view_desc.mip_count;
			...
			case GfxSubresourceType_RTV:
			{
				...
				else CASE_ENGINE_ASSERT(false && "Failed RTV Creation");
			}
			break;
			case GfxSubresourceType_DSV:
			{
				...
				else CASE_ENGINE_ASSERT(false && "Failed DSV Creation");
			}
			break;
			default:
				break;
