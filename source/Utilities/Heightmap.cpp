...
namespace Case_Engine
{
	...
	Heightmap::Heightmap(std::string_view heightmap_path, uint32_t max_height)
	{
		Image img(heightmap_path, 1);

		uint8_t const* image_data = img.Data<uint8_t>();

		hm.resize(img.Height());
		for (uint64_t z = 0; z < img.Height(); ++z)
		{
	...
	float Heightmap::HeightAt(uint64_t x, uint64_t z) const
	{
		return hm[z][x];
	}
	...
	{
		uint64_t drops = (uint64_t)desc.drops;

		uint64_t xresolution = Width();
		uint64_t yresolution = Depth();

		IntRandomGenerator<uint64_t> x_random(0, xresolution - 1);
		IntRandomGenerator<uint64_t> y_random(0, yresolution - 1);
		...
		for (uint64_t drop = 0; drop < drops; drop++)
		{
			...
			if (hm[Y][X] > 0.0f)
			{
				for (int32_t iter = 0; iter < desc.iterations; iter++)
				{
					float val = hm[Y][X];
					float left = 1000.0f;
					float right = 1000.0f;
