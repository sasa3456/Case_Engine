...
namespace Case_Engine
{
	...
	namespace 
	{
		std::optional<SceneConfig> ParseSceneConfig(const std::string &scene_file)
		{
			SceneConfig config{};
			...
			{
				CASE_ENGINE_LOG(ERROR, "JSON Parse error: %s! ", e.what());
				return std::nullopt;
			}
			...
			for (auto&& model_json : models)
			{
				...
				{
					CASE_ENGINE_LOG(WARNING, "Model doesn't have path field! Skipping this model...");
				}
			...
			for (auto&& light_json : lights)
			{
				...
				{
					CASE_ENGINE_LOG(WARNING, "Light doesn't have type field! Skipping this light...");
				}
				...
				light.mesh_size = light_params.FindOr<uint32_t>("size", 100u);
				...
				{
					CASE_ENGINE_LOG(WARNING, "Light has invalid type %s! Skipping this light...", type.c_str());
				}
			...
			else
			{
				...
				{
					CASE_ENGINE_LOG(WARNING, "Skybox texture not found or is incorrectly specified!  \
	...
	Engine::Engine(const EngineInit &init) : window(init.window), vsync{ init.vsync }, scene_viewport_data{}
	{
		...
		std::ignore = input_events.left_mouse_clicked.Add([this](int32_t mx, int32_t my) { renderer->OnLeftMouseClicked(); });
	...
	void Engine::OnWindowEvent(const WindowEventData &data)
	{
		g_Input.OnWindowEvent(data);
	}

	void Engine::Run(const RendererSettings &settings)
	{
		static Timer timer;
	...
	void Engine::Render(const RendererSettings &settings)
	{
	...
	void Engine::InitializeScene(const SceneConfig &config)
	{
